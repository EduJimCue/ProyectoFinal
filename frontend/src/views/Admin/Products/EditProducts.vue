    ...
    <div class="admin-content">
      <div class="edit-product">
        ...
        <b-form @submit.prevent="updateProduct">
          ...
          <b-form-group label="Activo:" label-for="isActive">
            <b-form-checkbox id="isActive" v-model="formData.isActive">Activo</b-form-checkbox>
          </b-form-group>
          <b-button type="submit">Actualizar Producto</b-button>
        </b-form>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    const product = this.adminProducts.find(product => product.id === productId);
    if (product) {
  ...
  methods: {
    updateProduct() {
      this.$store.dispatch('actualizeProduct', this.formData)
        .then(() => {
          this.$router.push('/adminProducts'); // Redirige a adminProducts después de actualizar el producto
        })
        .catch(error => {
